...
interface Props {
  ...
  action: { key: string };
  model: string;
  record: { id: string; name?: string; published?: boolean };
  endpoint?: string;
  onSuccess: () => void;
}

export function TogglePublishedModal({ open, onOpenChange, action, model, record, endpoint, onSuccess }: Props) {
  ...
  const nextState = record.published ? "despublicar" : "publicar";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-xl dark:bg-zinc-900">
        ...
        {record.name && <p className="mt-1 text-sm text-zinc-500">{record.name}</p>}

        {step === "confirm" && (
          <p className="mt-4 text-sm">¿Deseas <b>{nextState}</b> este producto?</p>
        )}
        ...
        {step === "success" && (
          <div className="mt-4 rounded border bg-green-50 p-3 text-sm dark:bg-green-950/30">Producto actualizado.</div>
        )}
