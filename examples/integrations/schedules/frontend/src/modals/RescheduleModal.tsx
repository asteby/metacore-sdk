...
interface Props {
  ...
  action: { key: string };
  model: string;
  record: { id: string; starts_at?: string; ends_at?: string };
  endpoint?: string;
  onSuccess: () => void;
}

export function RescheduleModal({ open, onOpenChange, action, model, record, endpoint, onSuccess }: Props) {
  ...
  useEffect(() => {
    if (open) { setStep("form"); setStarts(""); setEnds(""); setErrorMsg(""); }
  }, [open]);
  if (!open) return null;
  const valid = starts && ends;
  ...
  async function submit() {
    ...
    try {
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ starts_at: new Date(starts).toISOString(), ends_at: new Date(ends).toISOString() }),
      });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="w-full max-w-lg rounded-lg bg-white p-6 shadow-xl dark:bg-zinc-900">
        ...
        {record.starts_at && <p className="mt-1 text-xs text-zinc-500">Actual: {record.starts_at}</p>}

        {step === "form" && (
          <div className="mt-4 grid gap-3">
            <label className="grid gap-1 text-sm">
              <span className="font-medium">Nuevo inicio</span>
            ...
            <label className="grid gap-1 text-sm">
              <span className="font-medium">Nuevo fin</span>
        ...
        {step === "success" && (
          <div className="mt-4 rounded border bg-green-50 p-3 text-sm dark:bg-green-950/30">Cita reagendada.</div>
        )}
