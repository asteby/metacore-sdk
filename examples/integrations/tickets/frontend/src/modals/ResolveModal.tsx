...
export function ResolveModal({ open, onOpenChange, action, model, record, endpoint, onSuccess }: Props) {
  const [step, setStep] = useState<Step>("form");
  const [notes, setNotes] = useState("");
  const [errorMsg, setErrorMsg] = useState("");

  useEffect(() => { if (open) { setStep("form"); setNotes(""); setErrorMsg(""); } }, [open]);
  if (!open) return null;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="w-full max-w-lg rounded-lg bg-white p-6 shadow-xl dark:bg-zinc-900">
        <h2 className="text-lg font-semibold">Resolver ticket</h2>
          ...
          <div className="mt-4 grid gap-3">
            <label className="grid gap-1 text-sm">
              ...
              <textarea value={notes} onChange={(e) => setNotes(e.target.value)}
                className="rounded border px-3 py-2 dark:bg-zinc-800" rows={4}
        ...
        {step === "submitting" && (
          <div className="mt-6 flex flex-col items-center gap-2">
            ...
            <p className="text-sm">Resolviendo…</p>
          </div>
        )}

        {step === "success" && (
          <div className="mt-4 rounded border bg-green-50 p-3 text-sm dark:bg-green-950/30">Ticket resuelto.</div>
        )}
