...
async function doLoad(spec, addonKey, version) {
    ...
    if (!container) {
        throw new Error(`metacore: remoteEntry at ${spec.entry} did not register container ${globalName}`);
    }
 ...
 */
export function containerName(spec, addonKey) {
    if (spec.container && spec.container.length > 0) {
        return spec.container;
    }
    if (!addonKey) {
        throw new Error("metacore: cannot derive federation container name — " +
